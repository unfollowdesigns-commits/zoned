...
export function WaveField({ height = 0.86, opacity = 1, className }: WaveFieldProps) {
  ...
  React.useEffect(() => {
    ...
    // The palette, sampled by wave height: deep indigo through violet to a
    // cool blue. RE-SKIN: replace RAMP with five steps of your own accent,
    // darkest first. Keep them inside the palette (see the note below).
    //
    // The ramp used to run on into cyan and mint, and the crests are exactly
    // the points the eye lands on, so the brightest and most saturated colour
    // in the whole field was a green that appears nowhere else on the site. It
    // read as a dense green wedge behind the copy. Ending at blue keeps the
    // field inside the palette it is supposed to be supporting.
  ...
  return (
    <canvas
      ...
      style={{
        ...
        maskImage: "linear-gradient(to top, rgba(0,0,0,0.9) 8%, rgba(0,0,0,0.5) 46%, transparent 88%)",
        WebkitMaskImage: "linear-gradient(to top, rgba(0,0,0,0.9) 8%, rgba(0,0,0,0.5) 46%, transparent 88%)",
      }}
    />
  )
}
